...
declare
  current_user_id uuid := auth.uid(); current_plan text; current_status text; current_period_end timestamptz;
...
begin
  ...
  select plan, status, subscriptions.current_period_end into current_plan, current_status, current_period_end from public.subscriptions where user_id = current_user_id for update;
  if current_status is null or current_status not in ('active','trialing') or (current_period_end is not null and current_period_end + interval '1 hour' <= now()) then
  ...
  if current_plan = 'plus' and monthly_count >= 15 then
    return jsonb_build_object('allowed', false, 'code', 'monthly_limit', 'message', 'You have used this month''s 15 personalized routine credits. Your allowance resets next month, or you can switch to Unlimited.');
  end if;
