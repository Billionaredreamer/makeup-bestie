...
export function ScanCamera({ onPhoto, onBack, busy, message }: { onPhoto:(file:File)=>void; onBack:()=>void; busy:boolean; message:string }) {
  ...
  useEffect(()=>{
    ...
    navigator.mediaDevices.getUserMedia({video:{facingMode:"user"},audio:false}).then(media=>{
      ...
      stream=media;if(video.current)video.current.srcObject=media;
  ...
  return <main className="scan-camera"><video ref={video} autoPlay muted playsInline/><button className="camera-close" onClick={onBack} aria-label="Back">←</button><div className="scan-oval" aria-hidden="true"/><div className="scan-controls"><p role="status">{error||message||"Face forward in even light."}</p><button className="scan-shutter" onClick={capture} disabled={busy||!!error} aria-label="Take face photo"/><p>On device. Nothing is uploaded.</p><label className="scan-file">Choose a photo<input type="file" accept="image/*" disabled={busy} onChange={e=>{const file=e.target.files?.[0];if(file)onPhoto(file);}}/></label></div></main>;
}
