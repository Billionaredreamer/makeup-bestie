...
export default function App() {
  ...
  const stopVoice = useCallback(() => { microphone.current?.getTracks().forEach(track => track.stop()); microphone.current = null; peer.current?.close(); peer.current = null; if (audio.current) { audio.current.srcObject = null; audio.current.remove(); } audio.current = null; setVoiceActive(false); }, []);
  ...
  const startVoice = async () => {
    try { setFeedback("Connecting your voice bestie…"); const tokenRes = await fetch("/api/realtime-session", { method: "POST" }); const token = await tokenRes.json(); if (!tokenRes.ok) throw new Error(token.error); const ephemeral = token.value || token.client_secret?.value; if (!ephemeral) throw new Error("The voice session did not return a temporary connection token.");
      ...
      const mic = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } }); microphone.current = mic; mic.getTracks().forEach(t => pc.addTrack(t, mic)); const dc = pc.createDataChannel("oai-events"); dc.onopen = () => dc.send(JSON.stringify({ type: "response.create", response: { instructions: `Greet the user briefly, then coach the current ${lesson[step][0]} step. The current instruction is: ${lesson[step][1]}` } })); dc.onerror = () => setFeedback("The voice conversation was interrupted. Tap Start voice to reconnect.");
      const offer = await pc.createOffer(); await pc.setLocalDescription(offer); const sdp = await fetch("https://api.openai.com/v1/realtime/calls", { method: "POST", headers: { Authorization: `Bearer ${ephemeral}`, "Content-Type": "application/sdp" }, body: offer.sdp }); if (!sdp.ok) { const detail = await sdp.text(); throw new Error(detail.includes("model") ? "The configured voice model is unavailable." : "Voice connection failed. Please try again."); } await pc.setRemoteDescription({ type: "answer", sdp: await sdp.text() }); setVoiceActive(true); setFeedback("Voice coaching is live. Camera frames are still not shared automatically.");
