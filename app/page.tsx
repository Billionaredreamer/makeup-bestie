...
import { PLUS_MONTHLY_SESSION_LIMIT, UNLIMITED_ALLOWANCE_COPY, plusSessionsLeft, plusSessionsLeftCopy } from "@/lib/subscription-copy";
import {
...
type View = "confirm-features" | "follow-mode" | "finished" | "peek" | "pricing" | "home" | "discover" | "creator" | "my-looks" | "onboarding" | "face-scan" | "studio-intake" | "look-brief" | "session" | "import" | "profile";
...
function MakeupBestieExperience({account}:{account:LaunchAccount}) {
  ...
  const guestMergeUserId=useRef<string|null>(null);
  const mergeGuestProfile=account.saveProfile;
  ...
  const remainingPlusSessions=plusSessionsLeft(account.snapshot?.usage.tutorialAnalyses);
  ...
  const rememberOnboardingCache=useCallback((userId:string|null|undefined,next:OnboardingCache)=>{
    ...
    setOnboardingCache(next);
    writeOnboardingCache(userId,next);
  },[]);
  const updateOnboardingCache=useCallback((userId:string|null|undefined,change:Partial<OnboardingCache>)=>{
    const next={...onboardingCacheRef.current,...change};
    rememberOnboardingCache(userId,next);
    return next;
  ...
  useEffect(()=>{
    ...
    if(cloudUserId&&shouldChooseInitialView){onboardingCacheRef.current=migratedCache;setOnboardingCache(migratedCache);}
    if(cloudProfile){
      const cloudComplete=profileRecordIsComplete(cloudProfile);
      clearGuestProfileDraft();
      const hydratedCache={...migratedCache,profileComplete:cloudComplete};
      rememberOnboardingCache(cloudUserId,hydratedCache);
      const next=resolveLaunchStage({profileComplete:cloudComplete,subscriptionActive:subscriptionRecordIsActive(account.snapshot?.subscription),peekSeen:hydratedCache.peekSeen});
    ...
    if(account.configured&&account.user){
      ...
      if(guestProfileIsComplete(guestDraft)&&guestMergeUserId.current!==cloudUserId){
        guestMergeUserId.current=cloudUserId;
      ...
      if(guestProfileIsComplete(guestDraft)&&guestMergeUserId.current===cloudUserId)return;
      ...
      const emptyProfileCache={...migratedCache,profileComplete:false};
      rememberOnboardingCache(cloudUserId,emptyProfileCache);
      const next=resolveLaunchStage({profileComplete:false,subscriptionActive:subscriptionRecordIsActive(account.snapshot?.subscription),peekSeen:emptyProfileCache.peekSeen});
    ...
    if(!account.configured)try {
      ...
      onboardingCacheRef.current=cached;setOnboardingCache(cached);
  ...
  const onboardingView=view==="peek"||view==="onboarding"||view==="pricing"||view==="follow-mode";
  ...
  const continueFromFaceScan=async()=>{
    ...
    try{
      ...
      if(cameraUnavailable)beginRoutine("slides");else go("follow-mode");
  ...
  const finishPeek=()=>{
    updateOnboardingCache(account.user?.id,{peekSeen:true,profileComplete:serverProfileComplete});
  ...
  if(view==="confirm-features")return <main className="flow-screen polished-flow confirm-features-screen"><header className="flow-topline"><button className="back" onClick={()=>go("face-scan")}>← Retake photo</button><span>2 of 3</span></header><section className="flow-intro"><p className="eyebrow">Your Face Blueprint</p><h1>Make the analysis yours.</h1><p>We’ve mapped your proportions privately. Open any section and change what feels off.</p></section>{shape&&<label className="shape-field"><span>Face proportions</span><select value={shape} onChange={e=>setShape(e.target.value as FaceShape)}>{["heart","oval","round","square","oblong","diamond"].map(item=><option key={item}>{titleCase(item)}</option>)}</select></label>}{faceBlueprint&&<FaceBlueprintEditor value={faceBlueprint} onChange={setFaceBlueprint}/>}<p className="flow-status" role="status">{mapMessage}</p><button className="primary flow-primary" disabled={mapStatus!=="ready"||!faceBlueprint} onClick={()=>void continueFromFaceScan()}>Confirm features →</button></main>;
  ...
  if(view==="studio-intake"&&(lookUrl.trim()||lookFile))return <main className="flow-screen polished-flow makeup-bag-screen"><header className="flow-topline"><button className="back" disabled={lessonAnalyzing} onClick={()=>{go("home");setComposerOpen(true);}}>← Add tutorial</button><span>1 of 3</span></header><section className="flow-intro"><p className="eyebrow">Personalize the routine</p><h1>Your makeup bag.</h1><p>Choose what you own. We’ll use your products first and make missing steps easy to skip or substitute.</p></section><section className="bag-card"><div className="bag-card-heading"><b>Products you have</b><small>{ownedProducts.length} selected</small></div><div className="bag-chips">{productOptions.map(product=><button key={product} aria-pressed={ownedProducts.includes(product)} onClick={()=>setOwnedProducts(ownedProducts.includes(product)?ownedProducts.filter(item=>item!==product):[...ownedProducts,product])}>{ownedProducts.includes(product)?"✓ ":"+ "}{product}</button>)}</div></section>{lessonAnalyzing&&<div className="analysis-status" role="status"><span/><div><b>Analyzing your tutorial</b><small>{lessonStage}</small></div></div>}{lessonError&&<p className="error">{lessonError}</p>}<div className="flow-actions"><button className="outline" disabled={lessonAnalyzing} onClick={createBrief}>Skip</button><button className="primary" disabled={lessonAnalyzing||(!lookUrl.trim()&&!lookFile)} onClick={createBrief}>{lessonAnalyzing?"Analyzing…":"Continue →"}</button></div></main>;
  ...
  if (view === "profile") {
    ...
    return <>{nav}<main className="profile app-screen band-page">
      ...
      <div className="stat-row">
        ...
        <div><b>{account.snapshot?.subscription?.plan==="unlimited"?"Unlimited":remainingPlusSessions}</b><span>{account.snapshot?.subscription?.plan==="unlimited"?"Unlimited adaptations":`of ${PLUS_MONTHLY_SESSION_LIMIT} sessions left`}</span></div>
