...
export async function POST(req: NextRequest) {
  ...
  body.append("prompt", `Create a realistic makeup visualization on the person in the first image. Preserve their identity, facial structure, expression, skin texture, lighting, hairstyle, and background. Apply only makeup—do not reshape features, retouch skin, change age, ethnicity, body, or attractiveness. Inspiration: ${description}. Intensity: ${intensity}. If a second image is supplied, transfer its makeup style rather than the other person's identity. The result is an illustrative preview, not a guaranteed outcome.`);
